...
import { NFT_ATTRIBUTE_KEYS, NFT_ATTRIBUTE_VALUES, NFT_DESCRIPTION, NFT_IMAGE_URL, NFT_NAME, NFT_SYMBOL } from './common.js';

const DISTRIBUTE_NFTS_CSV_PATH = '';
const PER_CHUNK = 10;
...
const submitDistributeNFts = async (addresses) => {
    ...
    console.log(
        { response: res },
        'Operator distribute NFTs to addressess',
    );
}

const distributeNFTs = async () => {
    const addresses = [];
    await fs.createReadStream(DISTRIBUTE_NFTS_CSV_PATH)
        .pipe(csvParser())
        .on('error', (err) => {
